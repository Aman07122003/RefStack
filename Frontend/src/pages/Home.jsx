...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    // ── Shader passes & simulation (unchanged logic) ──────────────────────────
    class ShaderPass {
      constructor(props) {
        this.props = props || {};
  ...
  // ✅ FIX 1: LiquidEther now returns its canvas mount point
  return (
    <div
      ref={mountRef}
      className={className}
...
const HomePage = () => {
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      <div className="bg-blue-600" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <header>
          <Navbar />
        </header>
        ...
        <main className="flex-grow">
          ...
          <section aria-label="Key Features" className="py-12">
            <Freatures />
          </section>
          <section aria-label="Quick Action Links" className="py-12">
            <QuickAction />
          </section>
        </main>

        <footer className="h-[60px] flex items-center" role="contentinfo">
          <Footer />
        </footer>
      </div>
